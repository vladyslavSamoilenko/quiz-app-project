...
export default function Dashboard() {
  ...
  if (loading || !user) return <div className="text-center p-10 text-purple-600">Ładowanie panelu...</div>;
          ...
          <div key={q.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4 hover:shadow-md transition">
            ...
            <div className="flex-grow">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="font-bold text-gray-800 text-lg">{q.title}</h3>
              </div>
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <button 
                ...
                className="flex-1 sm:flex-none px-4 py-2 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded transition"
              >
                Edytuj
              </button>
              <button 
                onClick={() => handleDelete(q.id)}
                className="flex-1 sm:flex-none px-4 py-2 text-sm font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded transition"
              >
                Usuń
              </button>
            </div>
